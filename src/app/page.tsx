...
export default function Page() {
  ...
  async function draw() {
    try {
      ...
      setWinners(ws);
      // On fait tourner la roue sur le premier gagnant uniquement (cosmétique)
  ...
  // Segments de la roue (limités pour lisibilité)
  const wheelLabels = useMemo(() => {
    const max = 24;
    const base = members.slice(0, max).map((m) => m.name);
    // S'assurer que le nom du gagnant #1 est présent pour l'animation
    if (spinName && !base.includes(spinName)) {
      if (base.length < max) return [...base, spinName];
      base[0] = spinName; // remplace le 1er segment
    }
